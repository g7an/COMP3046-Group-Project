...
int main()                                                
{
	...
	vector<float> y_test;
	bool train = true;
	float **d_Xtrain, **d_Xtest, **d_Ytrain, **d_Ytest;
	...
	if(train){
		...
		if (myfile.is_open())
		{
			...
			cudaMalloc((void**)&d_Xtrain, X_train.size() * sizeof(float*));
			float *cpucpy[X_train.size()];
			for (int i = 0; i < X_train.size(); i++)
			{

				cudaMalloc(&cpucpy[i], 28*28*sizeof(float)); 

			}
			cudaMemcpy(d_Xtrain, cpucpy, 28*28*sizeof(float), cudaMemcpyHostToDevice);
